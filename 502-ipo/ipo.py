...
class Solution:
    def findMaximizedCapital(self, k: int, w: int, profits: List[int], capital: List[int]) -> int:
        ...
        for i in range(len(profits)):
            costs_heap.heappush((capital[i], profits[i]))

        while k > 0:
            while costs_heap.size > 0 and w >= costs_heap.heap_arr[1][0]:
                profits_heap.heappush(costs_heap.heappop()[1])
            if profits_heap.size == 0:
                break
            profit = profits_heap.heappop()
            w += profit
            k -= 1
        return w